...
export const BookController = {
    ...
    recommendations: async ({ params }: {
        ...
        try {
            ...
            // 1. ดึงข้อมูลหนังสือหลักเพื่อดู 'category'
            const mainBook = await prisma.book.findUnique({
                where: { id: bookId },
                select: { category: true, id: true }
            });

            if (!mainBook) {
                // หากไม่พบหนังสือหลัก ก็ไม่สามารถแนะนำได้
                return { error: 'Main book not found' };
            }

            // 2. แยกหมวดหมู่ของหนังสือหลัก (สมมติว่าเป็น comma-separated string)
            // ใช้หมวดหมู่แรกเป็นหลักในการค้นหา
            const mainCategory = mainBook.category
                ? mainBook.category.split(',').map(c => c.trim()).filter(c => c !== '')[0]
                : null;
            ...
            if (mainCategory) {
                // ตรรกะการแนะนำ: แนะนำหนังสืออื่นๆ ที่มีหมวดหมู่เดียวกัน (Item-to-Item)
                // เราจะใช้ `contains` ใน Prisma เพื่อค้นหาสตริงที่มีหมวดหมู่นี้
                recommendedBooks = await prisma.book.findMany({
                    where: {
                        // ไม่รวมหนังสือเล่มปัจจุบัน
                        NOT: {
                            id: mainBook.id 
                        },
                        // ค้นหาในฟิลด์ category ที่มีหมวดหมู่หลักอยู่
                        category: {
                    ...
                    // จำกัดจำนวนแนะนำไม่ให้มากเกินไป
                    take: 5 
                });
            } else {
                // ตรรกะสำรอง: หากไม่มีหมวดหมู่ ให้แนะนำหนังสือใหม่ๆ แทน
                recommendedBooks = await prisma.book.findMany({
                    where: {
                        NOT: { id: mainBook.id },
                        status: 'active'
