...
export const CartController = {
    ...
    upQty: async ({ params }: {
        params: {
            id: string // นี่คือ ID ของรายการในตะกร้า (Cart Item ID)
        }
    }) => {
        try {
            // 1. ค้นหารายการในตะกร้า และรวม (include) ข้อมูล Book ที่เกี่ยวข้องมาด้วย
            ...
            // 2. ตรวจสอบว่าพบรายการในตะกร้าหรือไม่
            if (!cartItem) {
                return { error: 'ไม่พบรายการในตะกร้า (Cart item not found)' };
            }
            
            // ตรวจสอบว่ามีข้อมูลหนังสือเชื่อมโยงมาหรือไม่
            if (!cartItem.book) {
                return { error: 'ไม่พบข้อมูลสินค้า (Book) ที่เชื่อมโยงกับรายการนี้' };
            }

            // 3. กำหนดค่าและตรวจสอบเงื่อนไขสต็อก
            const currentQtyInCart = cartItem.qty; // จำนวนปัจจุบันในตะกร้า
            const newQtyInCart = currentQtyInCart + 1;
            const availableStock = cartItem.book.qty; // จำนวนสต็อกที่มีในโมเดล Book

            // ตรวจสอบว่าจำนวนใหม่ในตะกร้าเกินสต็อกที่มีอยู่หรือไม่
            if (newQtyInCart > availableStock) {
                // ถ้าเกินสต็อก ให้ส่งข้อความแจ้งเตือนกลับไป
                return {
                    error: `ไม่สามารถเพิ่มจำนวนสินค้าได้อีก จำนวนคงเหลือในสต็อกคือ ${availableStock} เล่ม`,
                    currentQty: currentQtyInCart,
                    maxQty: availableStock
                };
            }

            // 4. ถ้าไม่เกินสต็อก ให้อัปเดตจำนวนในตะกร้า
            return await prisma.cart.update({
                data: {
                    qty: newQtyInCart
                },
            ...
            // จัดการข้อผิดพลาดในการดำเนินการ
            return { error: `เกิดข้อผิดพลาดในการอัปเดต: ${err.message}` };
        }
    },
